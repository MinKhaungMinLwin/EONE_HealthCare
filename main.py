...
async def chat(request: ChatRequest):
    ...
    # We look back to find the last user asked before the last reply
    previous_user_msgs = ""
    if len(request.messages) >= 3:
        # Index -1 is current user, -2 is bot, -3 is previous user
        if request.messages[-3].role == "user":
            previous_user_msgs = request.messages[-3].content


    # Check Language (for strict instruction)
    has_korean = re.search(r"[가-힣]", current_msg)
    if has_korean:
    ...
    combined_context = f"{previous_user_msgs} {current_msg}"

    # 2. Try to find a matching SQL Template
    qid, sql = resolve_intent(current_msg)
    if not sql and previous_user_msgs:
        # If current message is vague, check if combined context triggers SQL
        qid, sql = resolve_intent(combined_context)
        
    print(f"Resolved intent from combined context: {qid}")
    ...
    if sql:
        ...
        ans = analyze(current_msg, data, sql)
        return {"role": "assistant", "content": ans}
    
    # =====================================================
    # SCENARIO B: Normal Conversation (Context Aware)
    # =====================================================
    else:
        ...
        # 2. Build the full history
        # We take the history coming from Streamlit and convert it to dict format
        # Pydantic  models need. dict() or .model_dump() depending on version.
        # but here we can just read attributes since it's a list of objects.
